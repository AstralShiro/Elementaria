...
namespace Elementaria.NPCs.Bosses.Butterveils.ArcanumPhase
{
    ...
    public class ButterveilArcanum : ModNPC
    {
        ...
        {
            DisplayName.SetDefault("Butterveil, The Arcanum");
        ...
        public override void SetDefaults()
        {
            ...
            music = mod.GetSoundSlot(SoundType.Music, "Sounds/Music/ArcanumButterveil");
			...
			else if (npc.ai[0] == 0f)
			{
				...
				if ((num1320 < 500f || npc.ai[3] < 0f) && npc.position.Y < Main.player[npc.target].position.Y)
				{
					...
					if (npc.ai[3] == 0f && Main.netMode != 1)
					{
                        ...
                        int projectile = Projectile.NewProjectile(npc.Center, direction * 10, ModContent.ProjectileType<ArcanumBlast>(), 5,0, Main.myPlayer);
                    }
				...
				if (Main.netMode != 1)
				{
					...
					if (npc.ai[3] >= num1332)
					{
						...
						if (!WorldGen.SolidTile(i2, j2))
						{
							...
							Player target = Main.player[npc.target];
                        	Vector2 direction = (target.Center - npc.Center).SafeNormalize(Vector2.UnitX);
                        	int projectile = Projectile.NewProjectile(npc.Center, direction * 1, ModContent.ProjectileType<ArcanumBlast>(), 5,0, Main.myPlayer);
						}
					}
				}
				if (Main.netMode != 1)
