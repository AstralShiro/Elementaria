...
using Elementaria.NPCs.Bosses.Butterveils.FirstPhase.Transf7;
using Elementaria.Projectiles.Bosses;

namespace Elementaria.NPCs.Bosses.Butterveils.FirstPhase.Transf6
{
    [AutoloadBossHead]
    public class BossTransf6 : ModNPC
    {
        ...
        public override void SetDefaults()
        {
            ...
            music = mod.GetSoundSlot(SoundType.Music, "Sounds/Music/ButterveilDark");
        ...
        {
            NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, ModContent.NPCType<BossTransf7>(), 0, 0f, 0f, 0f, 0f);

            // Spawn 2nd Phase
            if (Main.netMode == NetmodeID.SinglePlayer) // Singleplayer
            {
                Main.NewText("You hearing the waves of the ocean", Color.Red);
            }
            else if (Main.netMode == NetmodeID.Server) // Server
            {
                NetMessage.BroadcastChatMessage(NetworkText.FromLiteral("You hearing the waves of the ocean"), Color.Red);
            }
        }
