...
using Elementaria.NPCs.Bosses.Butterveils.FirstPhase.Transf5;
using Elementaria.Projectiles.Bosses;

namespace Elementaria.NPCs.Bosses.Butterveils.FirstPhase.Transf4
{
    [AutoloadBossHead]
    public class BossTransf4 : ModNPC
    {
        ...
        public override void SetDefaults()
        {
            ...
            music = mod.GetSoundSlot(SoundType.Music, "Sounds/Music/ButterveilThunder");
        ...
        {
            NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, ModContent.NPCType<BossTransf5>(), 0, 0f, 0f, 0f, 0f);

            // Spawn 2nd Phase
            if (Main.netMode == NetmodeID.SinglePlayer) // Singleplayer
            {
                Main.NewText("You feeling the earth tremble", Color.Red);
            }
            else if (Main.netMode == NetmodeID.Server) // Server
            {
                NetMessage.BroadcastChatMessage(NetworkText.FromLiteral("You feeling the earth tremble"), Color.Red);
            }
        }
