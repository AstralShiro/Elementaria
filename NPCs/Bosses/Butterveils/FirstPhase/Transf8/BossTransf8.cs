...
using Elementaria.NPCs.Bosses.Butterveils.SecondPhase;
using Elementaria.Projectiles.Bosses;

namespace Elementaria.NPCs.Bosses.Butterveils.FirstPhase.Transf8
{
    [AutoloadBossHead]
    public class BossTransf8 : ModNPC
    {
        ...
        public override void SetDefaults()
        {
            ...
            music = mod.GetSoundSlot(SoundType.Music, "Sounds/Music/ButterveilHallow");
        ...
        {
            NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, ModContent.NPCType<ButterveilSecondPhase>(), 0, 0f, 0f, 0f, 0f);

            // Spawn 2nd Phase
            if (Main.netMode == NetmodeID.SinglePlayer) // Singleplayer
            {
                Main.NewText("You feel weird", Color.Red);
            }
            else if (Main.netMode == NetmodeID.Server) // Server
            {
                NetMessage.BroadcastChatMessage(NetworkText.FromLiteral("You feel weird"), Color.Red);
            }
        }
