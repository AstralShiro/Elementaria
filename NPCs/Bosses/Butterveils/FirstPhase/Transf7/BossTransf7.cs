...
using Elementaria.NPCs.Bosses.Butterveils.FirstPhase.Transf8;
using Elementaria.Projectiles.Bosses;

namespace Elementaria.NPCs.Bosses.Butterveils.FirstPhase.Transf7
{
    [AutoloadBossHead]
    public class BossTransf7 : ModNPC
    {
        ...
        public override void SetDefaults()
        {
            ...
            music = mod.GetSoundSlot(SoundType.Music, "Sounds/Music/ButterveilWater");
        ...
        {
            NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, ModContent.NPCType<BossTransf8>(), 0, 0f, 0f, 0f, 0f);

            // Spawn 2nd Phase
            if (Main.netMode == NetmodeID.SinglePlayer) // Singleplayer
            {
                Main.NewText("You seeing a strange light from the sky", Color.Red);
            }
            else if (Main.netMode == NetmodeID.Server) // Server
            {
                NetMessage.BroadcastChatMessage(NetworkText.FromLiteral("You seeing a strange light from the sky"), Color.Red);
            }
        }
