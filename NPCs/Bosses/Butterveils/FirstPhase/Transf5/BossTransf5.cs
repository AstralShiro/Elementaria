...
namespace Elementaria.NPCs.Bosses.Butterveils.FirstPhase.Transf5
{
    ...
    public class BossTransf5 : ModNPC
    {
        ...
        public override void SetDefaults()
        {
            ...
            music = mod.GetSoundSlot(SoundType.Music, "Sounds/Music/ButterveilEarth");
        ...
        {
            NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, ModContent.NPCType<BossTransf6>(), 0, 0f, 0f, 0f, 0f);

            // Spawn 2nd Phase
            if (Main.netMode == NetmodeID.SinglePlayer) // Singleplayer
            {
                Main.NewText("You feeling a power corrupting your heart", Color.Red);
            }
            else if (Main.netMode == NetmodeID.Server) // Server
            {
                NetMessage.BroadcastChatMessage(NetworkText.FromLiteral("You feeling a power corrupting your heart"), Color.Red);
            }
        }
