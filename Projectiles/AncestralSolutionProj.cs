...
namespace Elementaria.Projectiles
{
	public class AncestralSolutionProj : ModProjectile
	{
		...
		public override void AI() {
			...
			int dustType = ModContent.DustType<Dusts.AncestralDust>();
			...
			for (int k = i - size; k <= i + size; k++) {
				for (int l = j - size; l <= j + size; l++) {
					if (WorldGen.InWorld(k, l, 1) && Math.Abs(k - i) + Math.Abs(l - j) < Math.Sqrt(size * size + size * size)) 
					{
						...
						if (wall != 0) {
							Main.tile[k, l].wall = (ushort)ModContent.WallType<AncestralDirtWall>();
						...
						if (TileID.Sets.Conversion.Stone[type]) {
							Main.tile[k, l].type = (ushort)ModContent.TileType<AncestralStoneTile>();
							WorldGen.SquareTileFrame(k, l, true);
							NetMessage.SendTileSquare(-1, k, l, 1);
						}
						else if (TileID.Sets.Conversion.Grass[type])
						{
							Main.tile[k, l].type = (ushort)ModContent.TileType<AncestralGrassTile>();
							WorldGen.SquareTileFrame(k, l, true);
							NetMessage.SendTileSquare(-1, k, l, 1);
						}
						//If the tile is sand, convert to ExampleSand
						else if (TileID.Sets.Conversion.Sand[type]) {
							Main.tile[k, l].type = (ushort)ModContent.TileType<AncestralSandTile>();
							WorldGen.SquareTileFrame(k, l, true);
							NetMessage.SendTileSquare(-1, k, l, 1);
						}
						/*						
