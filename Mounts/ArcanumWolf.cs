...
{
	public class ArcanumWolf : ModMountData
	{
		public override void SetDefaults()
		{
			mountData.spawnDust = ModContent.DustType<ArcanumDust>();
			mountData.buff = ModContent.BuffType<ArcanumWolfBuff>();
		...
		{ 
			...
			Dust.NewDust(new Vector2(rect.X, rect.Y), rect.Width, rect.Height, DustType<ArcanumDust>());
		}

		internal class ArcanumWolfSpecificData
		{
			internal int count;
			internal float[] rotations;
			internal static float[] offsets = new float[] { 0, 14, -14 };
			public ArcanumWolfSpecificData()
			{
				count = 3;
