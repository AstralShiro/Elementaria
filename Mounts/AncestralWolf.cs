...
{
	public class AncestralWolf : ModMountData
	{
		public override void SetDefaults()
		{
			mountData.spawnDust = ModContent.DustType<AncestralDust>();
			mountData.buff = ModContent.BuffType<AncestralWolfBuff>();
		...
		{ 
			...
			Dust.NewDust(new Vector2(rect.X, rect.Y), rect.Width, rect.Height, DustType<AncestralDust>());
		}

		internal class AncestralWolfSpecificData
		{
			internal int count;
			internal float[] rotations;
			internal static float[] offsets = new float[] { 0, 14, -14 };
			public AncestralWolfSpecificData()
			{
				count = 3;
