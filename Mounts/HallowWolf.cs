...
{
	public class HallowWolf : ModMountData
	{
		public override void SetDefaults()
		{
			mountData.spawnDust = ModContent.DustType<HallowDust>();
			mountData.buff = ModContent.BuffType<HallowWolfBuff>();
		...
		{ 
			...
			Dust.NewDust(new Vector2(rect.X, rect.Y), rect.Width, rect.Height, DustType<HallowDust>());
		}

		internal class HallowWolfSpecificData
		{
			internal int count;
			internal float[] rotations;
			internal static float[] offsets = new float[] { 0, 14, -14 };
			public HallowWolfSpecificData()
			{
				count = 3;
